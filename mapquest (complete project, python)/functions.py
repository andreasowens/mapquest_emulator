...
def LATLNG(json_object: 'json object') -> str:
    ...
    for info in json_object['route']['locations']:
        display_latlng.append(info['latLng']['lat'])
        display_latlng.append(info['latLng']['lng'])
    print("{:.2f} {:.2f}".format(info['latLng']['lat'], info['latLng']['lng']))
    print()
    return list_of_latlng, display_latlng
